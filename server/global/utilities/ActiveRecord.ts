...
/* ----------------------------------------------------------------------------------
 * ActiveRecord class version 1.0.0
/* ---------------------------------------------------------------------------------- */
class ActiveRecord {
  protected model: any
  ...
  protected deleteOne = async <TModel, TFilter>(filter: TFilter): Promise<TModel> => {
    return new Promise((resolve: IResolve<TModel>, reject: IReject): void => {
      this.model.deleteOne(filter).exec((error: IMongooseError, result: TModel): void => {
        const errorMessageHeader = 'deleteOne'
        this.checkForErrorThenLogAndReject(error, reject, errorMessageHeader)
        resolve(result)
      })
    })
  ...
  protected find = async <TModel, TFilter>(
    ...
    const { limit, select, skip, sort }: IModelOptions = this.getValidQueryOptions(options)
  ...
  protected findOne = async <TModel, TFilter, TSelectedFields>(
    ...
    const { select }: IModelOptions = queryOptions
    return new Promise((resolve: IResolve<TModel>, reject: IReject): void => {
      this.model
        .findOne(filter)
        .select(select)
  ...
  protected count = async <TFilter>(filter: TFilter): Promise<number> => {
  ...
  protected generateMatchValuesFilter = (
    property: string,
    list: string[] = [],
  ): IActiveRecordMatchValues => {
  ...
  private getValidQueryOptionsForSingleDoc = (
    options: ISingleDocModelOptionsInput,
  ): ISingleDocModelOptions => {
    if (!options) {
      return {}
